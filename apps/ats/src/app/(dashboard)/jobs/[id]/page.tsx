...
export default function JobDetailPage() {
  ...
  const [showAiDesc, setShowAiDesc]         = useState(false); // controls source-email collapse
  ...
  // Auto-generate AI description when job loads and doesn't have one yet
  const autoGeneratedRef = React.useRef(false);
  useEffect(() => {
    if (!job || autoGeneratedRef.current) return;
    if (job.description && !job.aiDescription && !aiDescGenerating) {
  ...
  const openCeipalModal = () => {
    ...
    setCeipalDraft({
      title:               job.title,
      description:         job.aiDescription || job.description || '',  // AI-polished preferred
  ...
  const generateAiDescription = async () => {
    ...
    try {
      ...
      setJob(j => j ? { ...j, aiDescription: data.aiDescription } : j);
    } catch (e: any) {
      setAiDescError(e.message);
    } finally {
    ...
    <div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className={`flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-medium
                      ${job.aiDescription ? 'bg-violet-100 text-violet-700' : 'bg-amber-50 text-amber-600'}`}>
                      {job.aiDescription
                        ? <><Sparkles className="w-3 h-3" /> AI-polished</>
                        : <><Mail className="w-3 h-3" /> Raw email</>}
                    </span>
                  </div>
                  <textarea
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    {job.aiDescription
                      ? 'AI-polished description — all requirements and terms preserved verbatim. Edit if needed before posting.'
                      : 'Raw email content — generate an AI description on the job page for a cleaner Ceipal posting.'}
                  </p>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Job Description — AI-polished primary, source email collapsed below */}
          <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                {job.aiDescription && !aiDescGenerating && (
                  <span className="flex items-center gap-1 px-2 py-0.5 bg-violet-100 text-violet-700 rounded-full text-[10px] font-semibold">
                    <Sparkles className="w-2.5 h-2.5" /> AI-Polished
                  </span>
                )}
                {aiDescGenerating && (
                  <span className="flex items-center gap-1 px-2 py-0.5 bg-amber-50 text-amber-600 rounded-full text-[10px] font-medium">
              ...
              <div className="flex items-center gap-2">
                {job.aiDescription && (
                  ...
                  >
                    {aiDescCopied ? <Check className="w-3 h-3 text-green-600" /> : <Copy className="w-3 h-3" />}
                ...
                >
                  {aiDescGenerating ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                  {aiDescGenerating ? 'Generating…' : job.aiDescription ? 'Regenerate' : 'Generate AI'}
                </button>
              </div>
            </div>

            {/* Primary: AI-polished description */}
            {aiDescGenerating && !job.aiDescription ? (
              <div className="flex items-center gap-3 py-10 justify-center text-violet-400 bg-violet-50 rounded-xl border border-violet-100">
                <Loader2 className="w-5 h-5 animate-spin" />
                <span className="text-sm font-medium">Generating professional job description from source email…</span>
              </div>
            ) : job.aiDescription ? (
              <pre className="text-sm text-gray-800 bg-gray-50 border border-gray-200 rounded-xl p-4 whitespace-pre-wrap leading-relaxed font-sans overflow-y-auto max-h-[500px]">
                {job.aiDescription}
              </pre>
            ) : aiDescError ? (
              <div className="flex items-start gap-2 p-3 bg-red-50 border border-red-200 rounded-lg">
                ...
                <div>
                  <p className="text-xs font-semibold text-red-700">AI generation failed</p>
                  <p className="text-xs text-red-600 mt-0.5">{aiDescError}</p>
                </div>
              </div>
            ) : (
              <div className="flex flex-col items-center gap-3 py-10 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                <Sparkles className="w-6 h-6 text-gray-300" />
                <p className="text-sm text-gray-500">No AI description yet.</p>
                <button
                  onClick={generateAiDescription}
                  disabled={!job.description}
                  className="px-4 py-2 text-xs font-medium bg-violet-600 text-white rounded-lg hover:bg-violet-700 disabled:opacity-50"
                >
                  Generate from Source Email
                </button>
              </div>
            )}

            {/* Ceipal notice — shows which version will be posted */}
            <p className="text-[11px] text-gray-400 flex items-center gap-1">
              <Globe className="w-3 h-3" />
              {job.aiDescription
                ? 'AI-polished description will be used when posting to Ceipal.'
                : 'Source email will be used for Ceipal until AI description is generated.'}
            </p>
            ...
            <div className="border border-gray-200 rounded-xl overflow-hidden">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-gray-600">Source Email</span>
                  <span className="text-xs text-gray-400">— original email body</span>
                </div>
                <div className="flex items-center gap-2">
                  {!editingDesc && showAiDesc && (
                    <button
                      onClick={e => { e.stopPropagation(); setEditingDesc(true); setDescDraft(job.description || ''); }}
                      className="flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700 px-2 py-1 rounded hover:bg-gray-200"
                    >
                      <Edit3 className="w-3 h-3" /> Edit
                    </button>
                  )}
                  {showAiDesc ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
                </div>
              </button>
              {showAiDesc && (
                <div className="p-3">
                  {editingDesc ? (
                    ...
                    <pre className="text-xs text-gray-600 bg-white border border-gray-100 rounded-lg p-3 whitespace-pre-wrap leading-relaxed font-mono overflow-x-auto max-h-80 overflow-y-auto">
                      {job.description}
                    </pre>
                  ) : (
                    <p className="text-sm text-gray-400 italic py-2">No source email — click Edit to add one.</p>
                  )}
                </div>
              )}
            </div>
